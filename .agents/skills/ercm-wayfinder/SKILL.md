...
4. Resolve the question using the ticket type and required evidence. Apply the explicit owner-waiver rule in `docs/agents/research-workflow.md` when the user names a document-completeness or source/provenance/rights-record gate to omit; record `WAIVED_BY_OWNER`, its exact scope and the narrowed claim boundary rather than reporting the gate as passed. Preserve rejected alternatives, uncertainty, null findings, and project claim states.
