...
- `WAIVED_BY_OWNER`（2026-09-06）：当前用户明确授权 Issue #7 本次仅上传 Git 跟踪的公开文件，不要求本机具备 catalog 所列的本地论文 PDF 原件和原始数据文件。
- 本授权只把本次 push 的门禁从完整本地验证收窄为 `scripts/verify --public`；交付物仍是待评审的公开研究报告、来源元数据、模拟与测试，不包含第三方论文原件、真人数据或原始数据。
- 本授权不表示缺失材料已取得、已核验或具有再分发权，也不扩大下文的证据结论；任何依赖这些省略材料全文、来源事实或权利状态的后续任务，必须重新打开该门禁，恢复精确文件并运行 `scripts/sources doctor` 与完整 `scripts/verify`。

## 结论速览

- **核心结论：** 回放保真度不能压缩为“最终状态是否相同”。建议在 `BENCH-E0` 中分别检查事件结构、有序回放、逐事件状态轨迹、最终语义状态、时间一致性和跨环境差分，即组合使用 O1–O6 工程 Oracle。
- **证据与方法：** 结论来自 7 篇一手论文和 4 份官方规范的全文/章节核对，并由 3 个确定性 `DATA-D1`、`simulation-only` 工程模拟检验指标能否发现事件丢失/重复/乱序、最终状态 Oracle 盲点，以及三种排序字段的职责差异。
- **未决项：** 当前结果不能给出真实浏览器/设备上的容差，也不能支持任何认知、生态或真人效度声明；跨生产者合并规则、环境矩阵、状态投影、负向夹具和重复次数仍需在 #8 中冻结。
