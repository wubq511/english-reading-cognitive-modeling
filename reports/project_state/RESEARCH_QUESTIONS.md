...
> 状态：沟通辅助，2026-08-30 根据负责人书面 RQ list 建立。本项目的 canonical RQ 仍是上文 RQ0–RQ7；本节只负责翻译，不产生第二套研究问题。

负责人 RQ 与项目 RQ 的对应关系：

| 负责人 RQ（原文要点） | 对应项目 RQ 与资产 | 说明 |
| --- | --- | --- |
| RQ1）哪些题型便于通过交互记录 replay；rebuild 成功率约 70% 即可 | 项目 RQ1 + RQ2，按题型分层；输入是 Phase 0 题目资产（Wayfinder #3/#4） | "按题型统计 replay 成功率"可直接成为 `BENCH-E0/E1` 的分层指标 |
| RQ2）哪些交互对 replay 重要、必须收集 | 项目 RQ1 + Wayfinder #8 logging 验收合同；`COL-L0/L1` 采集层 | 方法即"重建倒推"：删去该类事件后状态时间线还能否确定性重建 |
| RQ3）大模型 replay 出的答题思路是什么 output 形式；如何据此用户建模 | 项目 RQ4（过程假设）+ RQ6（学生建模）的内容，但由 LLM 执行推断 | 含 LLM 推断，属项目 RQ7 扩展区间；须在无 AI baseline 冻结后按同数据/同 split/同 metric 比较 |
| RQ4）如何评估大模型 replay 质量；用评估倒推设计 | 工程层：项目 RQ1 / `BENCH-E0` 指标（Wayfinder #7）；认知层：项目 RQ5（构念效度） | 评估"答题思路"层面的质量必须有真人参照标注（如 stimulated recall），届时触发 H2+M1 门禁，无法绕过 |

**术语歧义提醒**：负责人说"replay"指*由大模型从交互记录推断学生答题思路*（认知层）；项目文档中 replay 指*从原始事件确定性重建 UI 状态*（工程层，不含认知）。两个含义都合法，但引用时必须区分，否则会把工程保真度指标错当成认知质量指标。

**路线顺序（已锁定，非分歧）**：负责人 RQ3/RQ4 含 LLM 是因为最终系统确定要引入 LLM；项目路线（AGENTS.md 任务第 1–2 条 + RQ7）早已锁定同一顺序——先做不含 LLM 的 baseline，其 interface、数据、split、metric、结果冻结后再上 AI 扩展。负责人 RQ3/RQ4 即该扩展阶段的目标表述，与项目路线一致。对当前工作只有两个操作含义：(a) Wayfinder #8 的日志合同必须保留 append-only 原始事件而非只存派生特征，否则未来 LLM replay 没有输入；(b) 评估 LLM replay 的认知层质量（答题思路对不对）届时需要真人参照标注，激活 H2+M1 门禁——那是扩展阶段的启动条件，不是现阶段负担。
